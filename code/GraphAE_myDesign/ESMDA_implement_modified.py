...
alpha = 0.06
Cd = 1*torch.eye(N_obs,dtype=torch.float32)
...
start = time.time()
a, b , Ensemble_M, Ensemble_d = esmda_model.assimilate (measurment_tensor)
end = time.time()
print(f"Runtime of the program is {end - start}")
print("a",torch.t(a).shape)
print("Latent_ensambels",Latent_ensambels[:,1].shape)
...
### PLot Histogram before and after of assimilation ######
path = "/home/yeganeh/Desktop/GNN-SOFA/train/graphAE_Breast/Plots"

a_t =torch.t(a)
hist_before_assimilation = torch.histogram(Latent_ensambels[:,1].cpu(), bins=300)
hist_after_assimiliation = torch.histc(a_t[:,1], bins=300)




pc_lst_sample0 = torch.unsqueeze(pc_lst[num+1,:],0).cuda()
pc_lst_sample = torch.unsqueeze(pc_lst[num,:],0).cuda()
latent_mu_g, latent_var =  model.encoder(pc_lst_sample0)
latent_mu, latent_var =  model.encoder(pc_lst_sample)
covarinace = torch.diag(torch.squeeze((0.5*latent_var).exp(),0))
m = MultivariateNormal(torch.squeeze(latent_mu,0), covarinace)
Latent_ensambels_gt = m.sample(torch.Size([1000]))
print("latent_mu_gt",latent_mu_g)
print("latent_mu",latent_mu)
print("latent_mu_modified",torch.mean(Ensemble_M[Na-1][1,:]))
for i in range(Na):
    # Show
    # plt.figure()
    plt.subplot(111)
    n, bins, patches = plt.hist(Latent_ensambels[:,1].cpu().numpy(), 300, density=False, facecolor='g', alpha=0.5)
    #plt.plot(hist_before_assimilation.numpy())
#    # plt.subplot(122)
    n, bins, patches = plt.hist(Ensemble_M[i][1,:].cpu().numpy(), 300, density=False, facecolor='r', alpha=1)
    #plt.plot(hist_after_assimiliation.cpu().numpy())
    plt.savefig(path+'/histogram'+str(i)+'.png')
...
plt.figure()
plt.subplot(111)
plt.hist(Latent_ensambels[:,1].cpu().numpy(), 300, density=False, facecolor='g', alpha=0.5)
plt.hist(Ensemble_M[Na-1][1,:].cpu().numpy(), 300, density=False, facecolor='r', alpha=1)
plt.hist(Latent_ensambels_gt[:,1].cpu().numpy(), 300, density=False, facecolor='b', alpha=0.1)
